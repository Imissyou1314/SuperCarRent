...
import com.wteamfly.supercarrent.biz.service.OrderService;
...
public class OrdersManageAction extends ActionSupport implements SessionAware {
	...
	private static Logger logger = LogManager
			.getLogger(OrdersManageAction.class);
	...
	public final void addOrders() {
		...
		if (tmporder.getBackCarTime() == null
				| tmporder.getGetCarTime() == null) {
			return;
		}
	 ...
	 * 通过用户Id来的获取用户订单.
	 */
	public final void findOrdersByUserId() {
		logger.debug("进入findOrdersByUserId方法");
		final PaginationPo paginationPo = page.voToPo(PaginationPo.class);
		final OrderService service = OrderService.getInstance();
		resultMessage = service.finalOrderByUserId(currentUser, paginationPo);
		resultMessage.setUserToken(true);

		logger.debug("退出findOrdersByUserId方法");
	}

	/**
	 * 查找特定订单状态的订单.
	 */
	public final void findOrdersByState() {
		logger.debug("进入findOrdersByState方法");
		final PaginationPo paginationPo = page.voToPo(PaginationPo.class);
		final OrderService service = OrderService.getInstance();
		resultMessage = service.finalOrdersByState(paginationPo);
		resultMessage.setUserToken(true);
		logger.debug("退出findOrdersByState方法");
	...
	public final void  findOrderDetailByOrderId() {
		logger.debug("进入findOrderDetailByOrderId方法");
		OrdersPo ordersPo = null;
		ordersPo = orders.voToPo(OrdersPo.class);
		OrderService service = OrderService.getInstance();
		resultMessage = service.findOrderDetailByOrderId(ordersPo);
		resultMessage.setUserToken(true);
